...
def main() -> int:
    ...
    parser.add_argument("--working-tree", action="store_true", help="Also inspect current edits and untracked source")
    ...
    if args.working_tree:
        changed = sorted(set(changed) | set(git('diff', '--name-only', 'HEAD').stdout.splitlines()) | set(git('ls-files', '--others', '--exclude-standard').stdout.splitlines()))

    diff_args = ("diff", "--check", base, head) if base else ("diff", "--check")
    diff_check = git(*diff_args, check=False)
    if args.working_tree:
        diff_check = git('diff', '--check', 'HEAD', check=False)
