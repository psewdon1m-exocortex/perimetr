#!/usr/bin/env python3
"""Revision-bound, fail-closed Part 12 release evidence gate for Perimetr."""
import argparse
import hashlib
import json
import os
...
def load_catalog(policy, directory):
    bundled = read_json(ROOT / ".release/known-problems-catalog.json")
    if bundled["revision"] != policy["catalog_revision"] or bundled["path"] != policy["remote_path"]:
        raise ValueError("Policy catalog identity mismatch")
    data = bundled["text"].encode("utf-8")
...
def main():
    ...
    identity = {"revision": revision, "catalog_sha256": policy["catalog_sha256"], "run_id": run_id, "source_sha256": source_fingerprint()}
    if args.record:
    ...
    if (qualification_dir / 'qualification.json').is_file():
        subprocess.run([sys.executable, str(ROOT / 'scripts/verify-qualification.py'), str(qualification_dir)], check=True, cwd=ROOT)
        qualified = {item['id']: item for item in read_json(qualification_dir / 'qualification.json')['checks']}
    checks = []
    for row in rows:
        problem_id = row["id"]
        ...
        if entry.get("not_applicable"):
            ...
            if good:
                status = "PASS"
            if problem_id in qualified and entry.get('phase') != 'final':
                status = qualified[problem_id]['status']
                evidence = qualified[problem_id]['evidence']
    ...
    if args.phase != "structure":
        expected = re.escape(policy["service"]) + r"-v(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*)"
        if not re.fullmatch(expected, tag) or tag.endswith('-v0.0.0'):
    ...
    report = {
        ...
        **identity,
        ...
        "deployment_activation": {
            "status": "NOT_RUN",
            "reason": "Production DNS/TLS, provider credentials, external reachability and installed helper versions require the operator activation checks in docs/deployment.md.",
        },
    }
